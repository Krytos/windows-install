...
function DownlaodInstallGithub($name, $repo, $filePattern) {
    ...
    try {
        ...
        Write-ColorOutput Red "An error occurred during download/install of $name: $($_.Exception.Message)"
    } finally {
...
function InstallPythonAndPackages {
    ...
    foreach ($version in $pythonVersions) {
        ...
        try { uv python install $version } catch { Write-ColorOutput Red "Failed to install Python $version: $($_.Exception.Message)"}
    }
    ...
    foreach ($tool in $pythonTools) {
        ...
        try { uv tool install $tool } catch { Write-ColorOutput Red "Failed to install Python tool '$tool': $($_.Exception.Message)"}
    }
    try { uv tool ensurepath } catch { Write-ColorOutput Red "Failed uv tool ensurepath: $($_.Exception.Message)"}
